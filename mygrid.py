...
class MyCellRenderer(wx.grid.GridCellRenderer):
    def __init__(self):
        super(MyCellRenderer, self).__init__()
        # Default background color
        #self.SetBackgroundColour(wx.Colour(255, 255, 255))
    
    # def Draw(self, grid, attr, dc, rect, row, col, isSelected):
    #     # Example drawing logic: Draw a red background if value contains '5'
    #     value = grid.GetCellValue(row, col)
    #     color = wx.Colour(255, 0, 0) if '5' in value else wx.Colour(255, 255, 255)
...
class MyGrid(wx.grid.Grid):
    def __init__(self, parent):
        ...
        self.Bind(wx.grid.EVT_GRID_CELL_CHANGED, self.on_change)
        ...
        #self.Bind(wx.grid.EVT_GRID_CELL_CHANGED, self.OnCellValueChanged)
        self.selected_rows = []
        self.selected_cols = []
        self.history = []
        self.invalid_dates = []
        self.SetCellRenderer(0, 0, MyCellRenderer())

    def CreateCellAttr(self, color):
        """Create and return a GridCellAttr with specified background color."""
        attr = wx.grid.GridCellAttr()
        attr.SetBackgroundColour(color)
        return attr
    
    def UpdateCellAttr(self, row, col, color):
        """Update the cell attribute for a specific cell."""
        #attr = self.CreateCellAttr(color)
        self.SetCellBackgroundColour(row, col, color)
        self.RefreshAttr(row, col)
        #self.SetCellAttr(row, col, attr)
    ...
    def on_change(self, event):
        ...
        self.UpdateCellAttr(row, col, bg_color)
        event.Skip()
        ...
        for row, line in enumerate(data.split("\n")):
            ...
            for col, value in enumerate(line.split("\t")):
                ...
                if value in self.invalid_dates:
                    ...
                    bg_color = wx.Colour(255, 255, 255)  # White background
                # Create a custom renderer and set the background color
                self.UpdateCellAttr(target_row, target_col, bg_color)
    ...
    def delete(self, event):
        ...
        for row, col in self.get_selected_cells():
            ...
            self.UpdateCellAttr(row, col, wx.Colour(255,255,255))
